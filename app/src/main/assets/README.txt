...
- int8 动态量化（int8 权重），体积约 25 MB；真机实测框质量与 FP32 无差异、速度快；搭配 ImageSlicer(tileSize=1280, overlap 25%) 使用

mahjong_11m_back_1280_fp32.tflite
...
正面 conf 0.45，牌背 conf 0.45，CPU/XNNPACK，双模型合计约 102 MB（≤500MB 预算；混合精度——正面 int8 + 牌背 FP32）。
